...
def test_funcs_with_docstring():
    assert bool(session10.calculate_age.__doc__) == True, "Docstring missing"
    assert bool(session10.timed_reps.__doc__) == True, "Docstring missing"
    assert bool(session10.create_random_profiles.__doc__) == True, "Docstring missing"
    assert bool(session10.d_create_random_profiles.__doc__) == True, "Docstring missing"
    assert bool(session10.stock_exchange.__doc__) == True, "Docstring missing"

def test_timed_reps_type_check():    
    with pytest.raises(ValueError) as e_info:
        _ = session10.timed_reps('text')

def test_create_random_profiles_type_check():    
    with pytest.raises(ValueError) as e_info:
        _ = session10.create_random_profiles('text')

def test_d_create_random_profiles_type_check():    
    with pytest.raises(ValueError) as e_info:
        _ = session10.d_create_random_profiles('text')
